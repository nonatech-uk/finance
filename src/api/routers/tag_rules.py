...
def reconcile_tag_rules(conn) -> dict:
    """Apply all active tag rules (full reconciliation).

    1. Delete all rule-generated tags for active rules
    2. For each active rule, insert matching tags
    3. ON CONFLICT DO NOTHING preserves manual tags

    Returns dict with rules_applied, tags_created, tags_removed.
    """
    cur = conn.cursor()
...
def reconcile_tag_rules_for_transactions(conn, transaction_ids: list[str]) -> None:
    """Re-apply active tag rules for specific transactions only.

    Deletes rule-generated tags for the given transactions, then re-inserts
    any that still match. Lightweight alternative to full reconciliation.
    """
    ...
    # Clear existing rule tags for these transactions
    cur.execute("""
        DELETE FROM transaction_tag
        WHERE raw_transaction_id = ANY(%s::uuid[])
        AND tag_rule_id IN (SELECT id FROM tag_rule WHERE is_active)
    """, (transaction_ids,))
    ...
    for rule_id, date_from, date_to, account_ids, merchant_pattern, category_pattern, tags in rules:
        conditions = ["rt.id = ANY(%(txn_ids)s::uuid[])"]
        params: dict = {"rule_id": rule_id, "tags": tags, "txn_ids": transaction_ids}
        joins = []
        has_merchant_join = False
...
def apply_tag_rules(
    ...
    return TagRuleApplyResult(**result)
